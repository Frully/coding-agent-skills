---
name: your-skill-name
description: Briefly state what this skill does and when an agent should use it.
---

# Skill Title

## Overview

<!-- Explain the workflow this skill covers and the outcome it should produce. -->

## When to use

<!-- List the situations, inputs, or user intents that should trigger this skill. -->

## When not to use

<!-- State nearby cases that should use a different skill or default behavior. -->

## Instructions

<!-- Write imperative, ordered instructions. Keep them specific about inputs, outputs, sequence, and boundaries. -->
<!-- Prefer portable shell commands and plain text steps. Avoid assuming Python, Node.js, or other environment-specific runtimes unless they are essential and explicitly documented. -->

## Examples

<!-- Prefer short input/output examples that show what good execution looks like. -->

## Additional files

<!-- Reference optional files only when needed:
- `references/` for long supporting docs
- `scripts/` for deterministic execution
- `assets/` for templates or static resources
- `agents/openai.yaml` for required Codex-specific metadata
Avoid adding scripts that require Python, Node.js, or other environment-specific runtimes unless the dependency is essential and clearly called out in the skill.
-->
